...
%%
[~,it]=min(abs(rp3_x-2.6));
for i=1:1%length(br_symmetry_wbifs(3).point)
pt=br_symmetry_wbifs(3).point(it);
figure(707)
clf;
plot(pt.mesh*pt.period,pt.profile(1:2,:),'LineWidth',1)
hold on 
%plot(pt.mesh*pt.period,pt.profile(3,:),'k','LineWidth',1)
%plot(pt.mesh*pt.period,pt.profile(4,:),'m','LineWidth',1)
grid on
...
%% looking for double spikes
for i=1:length(br_symmetry_wbifs(2).point)
pt2=br_symmetry_wbifs(2).point(i);
figure(78)
clf;
plot(pt2.mesh,pt2.profile(1:2,:),'LineWidth',1)
grid on
end
